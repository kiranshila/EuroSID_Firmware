...
void writeVoiceFreq(SID *sidChip, int voice) {
	...
	writeSID(sidChip,
		VOICE_1_FREQ_LOW + (voice * VOICE_REGISTER_OFFSET),
		sidChip->_voices[voice]._freq & 0xFF);

	writeSID(sidChip,
		VOICE_1_FREQ_HIGH + (voice * VOICE_REGISTER_OFFSET),
...
void writeVoicePWM(SID *sidChip, int voice) {
	...
	// Frequency is two bytes
	writeSID(sidChip,
		VOICE_1_PWM_LOW + (voice * VOICE_REGISTER_OFFSET),
		sidChip->_voices[voice]._pwm & 0xFF);

	writeSID(sidChip,
		VOICE_1_PWM_HIGH + (voice * VOICE_REGISTER_OFFSET),
...
void writeVoiceEnv(SID *sidChip, int voice) {
	...
	writeSID(sidChip,
		VOICE_1_ATTACK_DECAY + (voice * VOICE_REGISTER_OFFSET),
		attack_decay);
	writeSID(sidChip,
		VOICE_1_SUSTAIN_RELEASE + (voice * VOICE_REGISTER_OFFSET),
		sustain_release);
}
